...
public interface TransportEquipmentEventRepository extends ExtendedRepository<TransportEquipmentEvent, UUID> {
    ...
    @Query("SELECT * FROM \"dcsa_v1_2\".transport_equipment_event a WHERE (:eventType IS NULL or a.event_type =:eventType) AND (:equipmentReference IS NULL or a.equipment_reference =:equipmentReference) ")
