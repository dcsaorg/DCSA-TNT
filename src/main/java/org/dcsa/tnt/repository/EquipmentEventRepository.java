...
public interface EquipmentEventRepository extends ExtendedRepository<EquipmentEvent, UUID> {
    ...
    @Query("SELECT * FROM \"dcsa_v2_0\".equipment_event a WHERE (:eventType IS NULL or a.event_type =:eventType) AND (:equipmentReference IS NULL or a.equipment_reference =:equipmentReference) ")
